...
estSOC_GPP_rasters <- list.files("Mapping/Map_data_out/pset_RCrk_GPP10_map_data/SOC_rasters", pattern='\\.tif$')
estSOC_rasters <- list.files("Mapping/Map_data_out/estSOC", pattern='\\.tif$')



###Build stack of ensemble pair difference rasters
e1_diff <- raster(paste0("Mapping/Map_data_out/pset_RCrk_GPP10_map_data/SOC_rasters/",estSOC_GPP_rasters[1])) -
...
for (i in 2:length(estSOC_rasters)) {
  r_diff <- raster(paste0("Mapping/Map_data_out/pset_RCrk_GPP10_map_data/SOC_rasters/",estSOC_GPP_rasters[i])) -
...
est_SOC_diff_range2[est_SOC_diff_range > 2] <- 2

plot(mask(est_SOC_diff_range2, agLand, inverse=TRUE), axes=FALSE, box=FALSE,
     main = "",
     add = FALSE,
     #breaks=seq(-0.1, 2, length.out=100),
...
writeRaster(estSOC_diff_mean_crop, paste0(save_path, "scenGPP10_diff_mean.tif"), options=c('TFW=YES'))
writeRaster(estSOC_diff_sd_crop, paste0(save_path, "scenGPP10_diff_sd.tif"), options=c('TFW=YES'))
...
estSOC_diff_mean_crop2[estSOC_diff_mean_crop > 1] <- 1
estSOC_diff_mean_crop2[estSOC_diff_mean_crop < 0] <- 0

#create a color scale
#create a color scale
pal <- colorRampPalette(c("#f6eff7",
  "#bdc9e1",
  "#67a9cf",
  "#02818a"))

pal2 <- colorRampPalette(c("#ffffca",
                           "#6cd571",
                           "#253494"))

# Setup to save plot as png
png(file="C:/github/MIMICS_HiRes/Figures/Fig6_Scenario maps/GPP10_estSOC_pair-diff_mn_map.png", width=4000, height=6000, units="px", res=600)
...
# Setup to save plot as png
png(file="C:/github/MIMICS_HiRes/Figures/Fig6_Scenario maps/GPP10_estSOC_pair-diff_sd_map.png", width=4000, height=6000, units="px", res=600)

estSOC_diff_sd_crop2 <- estSOC_diff_sd_crop
estSOC_diff_sd_crop2[estSOC_diff_sd_crop > 0.3] <- 0

#overlay raster
plot(estSOC_diff_sd_crop2, axes=FALSE, box=FALSE,
     main = "",
     add = FALSE,
     breaks=seq(0, 0.5, length.out=600),
     col = viridis(600, option = "A", direction = -1), #pal(100),
     alpha = 1,
