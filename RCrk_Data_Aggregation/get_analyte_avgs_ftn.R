...
source("analyte_avg_by_loc_ftn2.R")
...
get_analyte_avgs <- function(target_database, target_analyte_to_avg, target_depth_cm) {
  ...
  get_loc_stocks <- lapply(unique(analyte_avgs_all$latlong), analyte_avg_by_loc, analyte_data = analyte_avgs_all, max_depth=target_depth_cm, trim="cut") #vs "loess"
...
#analyte_avgs_all <- analyte_summary(database=dt1, analyte="lyr_soc_stock")
