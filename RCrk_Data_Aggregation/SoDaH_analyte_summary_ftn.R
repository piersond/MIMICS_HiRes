...
analyte_summary <- function(database, analyte) {
  ...
  df <- database %>% select(site_code, location_name, lat, long, latlong, sample_collector, layer_top, layer_bot, analyte) #<-- lyr_soc (i.e. analyte of interest) needs to come from function input of dataframe column 
  ...
  df_smry <- df %>% mutate(site_code = as.character(site_code),
                          ...
                          sample_collector = as.character(sample_collector)) %>%
    group_by(site_code, location_name, latlong, sample_collector, layer_top, layer_bot) %>%
